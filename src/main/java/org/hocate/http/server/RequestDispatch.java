...
public class RequestDispatch {
	...
	public void ExceptionMessage(HttpRequest request,HttpResponse response,Exception e){
		...
		if(errorDefine.containsKey(className)){
			...
			if(TString.isNullOrEmpty(error.get("Description").toString())){
		...
		errorPageContent = errorPageContent.replaceAll("\\{\\{StatusCode\\}\\}", error.get("StatusCode").toString());
		errorPageContent = errorPageContent.replaceAll("\\{\\{RequestMethod\\}\\}", Matcher.quoteReplacement(requestMethod));
		errorPageContent = errorPageContent.replaceAll("\\{\\{RequestPath\\}\\}", Matcher.quoteReplacement(requestPath));
		errorPageContent = errorPageContent.replaceAll("\\{\\{ErrorMessage\\}\\}", Matcher.quoteReplacement(errorMessage));
		errorPageContent = errorPageContent.replaceAll("\\{\\{Description\\}\\}", Matcher.quoteReplacement(error.get("Description").toString()));
		errorPageContent = errorPageContent.replaceAll("\\{\\{Version\\}\\}", Matcher.quoteReplacement(Config.getVersion()));
